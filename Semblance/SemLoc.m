...
function openfolder(~,~)
    ...
    [ff,~]= getAllFiles(pathnam,search_mat);
        ...
        for i=1:len
            ...
            if (nlist==NStation)
                ...
                if delim==3
                    ...
                    new_figure=figure('units','normalized','outerposition',[0 0 1 1],'Name','Select window analysis','NumberTitle','off');
                    YY=ZZ(:,staidx);
                    YY=YY./max(YY);
                    plot(YY)
                    ylim([-2 2])
...
function save_sem(~,~)
    ...
    for kk=1:size(LOCAL,1)
        ...
        %Compute the temporal variables 
         ...
         Time=datenum(str2num(yy),0,str2num(jday)+str2num(hh)/24);
