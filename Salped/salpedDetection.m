...
function openfolder(~,~)
    ...
    [ff,~]= getAllFiles(pathnam,search_sac);
...
function calc2(~,~)
    try
        ...
        if ~isempty(pathnam);pathname=pathname2{i};pathname=[pathname '/'];end;path=pathname(1:end-4);
    ...
    try
        ...
        switch params.comp_chan
            case'Comp'
                ...
                path=params.coord;
                %Control and set the Input folder
                if ~isempty(pathnam);pathname=pathname2{i};pathname=[pathname '/'];end;mainpath=pathname(1:end-4);
