...
Fig1= figure('Name', 'Beam Pattern', 'position',get(0,'screensize'), ...
    'NumberTitle','off','units','normalized', 'color', fig_color);%%Create the map and beam pattern figure
...
colorbar(axes2)
xlabel(axes2,'S_x (2 \pi km^{-1})', 'fontsize', 8);
ylabel(axes2,'S_y (2 \pi km^{-1})', 'fontsize', 8);
        ...
        if ~isempty(params.map) 
            ...
            switch params.coord_sys
                case 'm'
                    ...
                    if isempty(xs);set(text1, 'String','Error. No correct coordinate file.');drawnow;return;end
                    cla(axes1)%Clear the Map axes
                    load('mapfile_m.mat')%Load the DEM variables (Utm system)
                    %Plot the DEM 
                ...
                case 'degrees'
                    ...
                    if isempty(LAT);set(text1, 'String','Error. No correct coordinate file.');drawnow;return;end
                    cla(axes1)%Clear the Map axes
                    load('mapfile_deg.mat')%Load the DEM variables (wgs84 system)
                    %Plot the DEM 
...
function ploting2(ii,ax)
    ...
    xlabel(ax,'S_x (2 \pi km^{-1})', 'fontsize', 7);
    ylabel(ax,'S_y (2 \pi km^{-1})', 'fontsize', 7);
...
function save1(~,~)
    ...
    xlabel('S_x (2 \pi km^{-1})', 'fontsize', 7);
    ylabel('S_y (2 \pi km^{-1})', 'fontsize', 7);
