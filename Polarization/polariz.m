...
h.rad2 = uicontrol( h.rad, 'Style','Radio','String','N',...
    'BackgroundColor',panel_color, 'units','normalized','position',[.18 0 .15 1],'HandleVisibility','off');
h.rad3 = uicontrol( h.rad, 'Style','Radio','String','E',...
    'BackgroundColor',panel_color, 'units','normalized','position',[.36 0 .15 1],'HandleVisibility','off');
h.rad4 = uicontrol( h.rad, 'Style','Radio','String','F',...
    'BackgroundColor',panel_color, 'units','normalized','position',[.54 0 .15 1],'HandleVisibility','off');
...
function ploting1(ii,ax)
    try
        ...
        az_mean= movmean(Az,hours(factor),'SamplePoints',Time);%Moving average of the azimuth according the factor value 
        ...
        curve1=az_mean+std(az_mean);
        curve2=az_mean-std(az_mean);
...
function ploting2(ii,ax)
    try
        ...
        time2=[Time', fliplr(Time')];
        curve1=inc_mean+std(inc_mean);
        curve2=inc_mean-std(inc_mean);
        inBetween = [curve1', fliplr(curve2')];
...
function ploting3(ii,ax)
    try
        ...
        time2=[Time', fliplr(Time')];
        curve1=ret_mean+std(ret_mean);
        curve2=ret_mean-std(ret_mean);
        inBetween = [curve1', fliplr(curve2')];
...
function save1(~,~)
    ...
    try
        ...
        time2=[Time', fliplr(Time')];
        curveaz1=az_mean+std(az_mean);
        curveaz2=az_mean-std(az_mean);
        inBetweenaz = [curveaz1', fliplr(curveaz2')];
         %Compute the standard deviation of the incidence
        curveinc1=inc_mean+std(inc_mean);
        curveinc2=inc_mean-std(inc_mean);
        inBetweeninc = [curveinc1', fliplr(curveinc2')];
        %Compute the standard deviation of the rectlinearity
        curveret1=ret_mean+std(ret_mean);
        curveret2=ret_mean-std(ret_mean);
